...
function build_people(file_name)
    ...
    nb_depots = parse(Int, data[3 + nb_client])
    depots = []

    for i in 1:nb_depots
        start_point, start_time, end_time = split(data[3 + nb_client + i], " ")
        start_point = parse(Int, start_point)
        start_time = parse(Int, start_time)
        end_time = parse(Int, end_time)
...
function new_people(people, gare, depots)
    new_people = people[:]
    append!(new_people, depots)
    push!(new_people, gare)
    return new_people
end

function new_people_cluster(people, gare, depot)
    new_people = people[:]
    push!(new_people, depot)
    push!(new_people, gare)
    return new_people
end
