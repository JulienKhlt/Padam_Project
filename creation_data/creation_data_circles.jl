...
function creation_data(file_name::String, file_loc_name::String, file_people_name::String, file_driver_name::String, nb_cercle, nb_point, rayons, nb_people, nb_bus, len) #nb_point = tableau de nombre de points pour chaque cercle
    open(file_name, "w") do file
        open(file_loc_name, "w") do file_loc
            open(file_people_name, "w") do file_people
                open(file_driver_name, "w") do file_driver
                    push_circles!(file, file_people, file_driver, file_loc, nb_cercle, nb_point, rayons, nb_people, nb_bus, len)
                end
            end
        end
    end
...
function push_circles!(file, file_people, file_driver, file_loc, nb_cercle, nb_point, rayons, nb_people, nb_bus, len)
    ...
    points, drivers_index = creation_point_circles(nb_cercle, rayons, nb_point, len)
...
function creation_point_circles(nb_cercle, rayons, nb_point, len)
    drivers_index = [1]
    points = []
    center_x = 0
    center_y = 0
...
creation_data("Data/circle/mTSP_matrix.csv", "Data/circle/node_coordinates.csv","Data/circle/customer_requests.csv", "Data/circle/driver_shifts.csv",3, [10,20,20], [1,3,5], 30, 5, 3000)
